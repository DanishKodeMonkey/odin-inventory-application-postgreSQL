...
const initItemsTable = async () => {
    ...
    try {
        ...
        await client.query(`
            ...
            category_ids INTEGER[] REFERENCES categories(id) ON DELETE SET NULL,
            description TEXT,
        ...
        for (let item of items) {
            await client.query(
                `INSERT INTO items(name, category_ids, description, price, numberInStock) VALUES($1, $2, $3, $4, $5) ON CONFLICT(name) DO NOTHING`,
                [
                    item.name,
                    item.category_ids,
                    item.description,
